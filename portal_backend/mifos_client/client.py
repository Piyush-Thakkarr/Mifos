...
class MifosClient:
    ...
    def auth_check(self) -> None:
        ...
        for idx, attempt in enumerate(attempts):
            ...
            try:
                # Use attempt-specific timeout if provided, otherwise default to 5 seconds (faster failure for local dev)
    ...
    def fetch_with_admin(self, path: str, method: str = "GET", params: Optional[Dict[str, Any]] = None, json: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            is_schedule_calculation = (
                ...
                params and "command" in params and params.get("command") == "calculateLoanSchedule"
            )
            is_loan_submission = (
                method == "POST" and "loans" in path and
                (not params or "command" not in params or params.get("command") != "calculateLoanSchedule")
            )
